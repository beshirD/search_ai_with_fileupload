...
import { getDocumentations } from 'apiSdk/documentations';
...
function DashboardPage() {
  ...
  const documentationStat = useSWR<PaginatedInterface<DocumentationInterface>>(
    () => `/documentations`,
    () => getDocumentations({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: documentationStat?.data?.totalCount,
      isLoading: documentationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
