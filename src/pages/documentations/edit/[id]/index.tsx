...
import { getDocumentationById, updateDocumentationById } from 'apiSdk/documentations';
...
function DocumentationEditPage() {
  const router = useRouter();
  const id = router.query.id as string;

  const { data, error, isLoading, mutate } = useSWR<DocumentationInterface>(
    () => (id ? `/documentations/${id}` : null),
    () => getDocumentationById(id),
  );
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: DocumentationInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateDocumentationById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<DocumentationInterface>({
    initialValues: data,
    validationSchema: documentationValidationSchema,
